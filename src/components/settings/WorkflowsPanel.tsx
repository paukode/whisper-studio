...
/**
 * Saved workflows manager. Workflows land here via the assistant's
 * `workflow_save` tool and are untrusted by default: running one by name
 * shows an approval preview card each time, and nested `workflow(name)`
 * calls from other scripts refuse to load it. Trusting a workflow records
 * its current script hash, so both paths run it without a preview until
 * the script changes.
 */
