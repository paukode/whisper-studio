...
# web_search / web_fetch ship in the native pool (server/executors/tools.py),
# so 'all' scope gets them from assemble_tool_pool like every other tool. The
# 'core_web' scope re-admits just these two names on top of CORE_TOOL_NAMES.
...
def get_tool_schemas(
    ...
    """The tool pool in OpenAI function format, filtered by ``scope``:
      - 'all'      — full cloud pool (parity; heaviest prompt)
    ...
    if scope == "core":
        ...
        pool = [t for t in pool if t["name"] in CORE_TOOL_NAMES | _WEB_TOOL_NAMES]
