...
WSDiagram.mount("ultracode-diagram", {
  ...
  nodes: [
    { id: "author", group: "model", col: 0, row: 0, label: "Model authors script", sub: "export const meta {…}", desc: "In the ultracode tier the model writes a deterministic JS orchestration script: a pure meta literal (name, description, phases) plus a body that calls agent() / pipeline() / parallel() / phase() / log(), and tools.<name>() for read-only tool calls with no model in the loop." },
    { id: "tool", group: "server", col: 1, row: 0, label: "workflow_run", sub: "parse + preview", desc: "server/workflows/tools.py spawns Node once to extract and validate the meta literal, then consults launch_policy.py: the workflow category mode can launch a new script directly (bypassPermissions, or auto within the 600k-token default budget) or decline it (dontAsk); otherwise it returns the approval preview side-effect. A trusted saved workflow or a resume launches immediately." },
    { id: "preview", group: "browser", col: 2, row: 0, label: "Approval preview", sub: "phases + full script", desc: "WorkflowPreviewCard shows the phases as a stepper, the whole script, and the output-token budget. Upfront human approval of every new script is the PRIMARY trust anchor for the vm. Its buttons are Approve & run and Deny. Approving POSTs to the launch route, and approving IS trusting: it grants that script hash for the session and, when the bytes match a saved workflow of the same name, marks that saved script trusted so future runs by name skip the card." },
    ...
    { id: "runtime", group: "server", col: 2, row: 1, label: "WorkflowRun", sub: "sem 16 · cap 1000 · 600k tok", desc: "server/workflows/runtime.py owns the limits the script cannot bypass: a 16-way concurrency semaphore, a 1000-agent lifetime cap, an output-token budget (600k default) re-checked on each slot, and the resume cache. It is the JSON-RPC server for the harness." },
    ...
    { id: "card", group: "browser", col: 4, row: 2, label: "WorkflowRunCard", sub: "live phase · cost · stop", desc: "The SPA renders the current phase name, agent count, and spend with Stop while running, Refresh once terminal (workflowStore, keyed by run_id). Resuming a run is model-initiated - workflow_run(resume_from_run_id=...) - not a card button." }
  ],
  edges: [
    { from: "author", to: "tool" },
