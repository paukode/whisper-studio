...
WSDiagram.mount("config-layers-diagram", {
  ...
  nodes: [
    ...
    { id: "user", group: "persist", kind: "store", col: 0, row: 2, label: "User", sub: "config.user.json", desc: "Your per-machine deltas for all sessions: keys, region, added models, per-field overrides, a hide-list. Under ~/.whisper for the Mac app, at the repo root for a source install. An older checkout with a monolithic config.json keeps that as its user layer until it is migrated. Gitignored; holds the Tavily key." },
