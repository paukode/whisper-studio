...
WSDiagram.mount("memory-diagram", {
  ...
  nodes: [
    ...
    { id: "wsmem", group: "persist", kind: "store", col: 3, row: 1, label: "Global + project memory", sub: "MEMORY.md + topics", desc: "Two tiers, same file shape: data/global_memory/ (always, even with no workspace open) and data/memory/<slug>/ (per workspace) - each a MEMORY.md index plus per-topic markdown with YAML frontmatter. Injected as <memory-context>." },
