...
WSDiagram.mount("hooks-diagram", {
  ...
  nodes: [
    ...
    { id: "stop", group: "security", col: 0, row: 0, label: "Stop gate", sub: "check_stop_hooks", desc: "At end of turn on the interactive chat policy only, via the completion gate, check_stop_hooks() can refuse to end the turn (a stop_hook_block frame), bounded by MAX_STOP_BLOCKS_PER_TURN so it can't loop forever. This is where the WS-E goal gate registers. Sub-agents, cron, and headless turns run with this gate off." },
