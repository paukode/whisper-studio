...
WSDiagram.mount("backends-diagram", {
  ...
  nodes: [
    ...
    { id: "local", group: "local", col: 2, row: 0, label: "LocalAdapter", sub: "llama-server / MLX", desc: "server/chat/engine/local.py: lean prompt, one resident model server (llama-server for GGUF, mlx_lm for MLX) dispatched by server/local/serving.py, tool-call fidelity, offline LOCAL policy (no completion gate, no permission-explainer model)." },
    ...
    { id: "openai", group: "external", kind: "external", col: 3, row: 1.5, label: "OpenAI adapter", sub: "GPT on mantle", desc: "server/chat/engine/openai.py: Responses API over bedrock-mantle, early-release + heartbeat quirks, prompt-too-long detection against the model's real window." },
