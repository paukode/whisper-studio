...
WSDiagram.mount("modes-flow-diagram", {
  ...
  nodes: [
    ...
    { id: "hybrid", group: "server", col: 1, row: 1, label: "Hybrid", sub: "per capability", desc: "Pick a backend per capability (embed, rerank, ner, index_llm). Unset capabilities fall back to the cloud backend. The on-device index/RAG weights download on first use." },
    { id: "local", group: "local", col: 1, row: 2, label: "Local", sub: "Qwen3 · GLiNER", desc: "The first-run default. Index/RAG stack on-device: Qwen3 embeddings, the Qwen3 Reranker, and GLiNER for entities. A few GB of weights, downloaded on your first index build; no Bedrock calls for indexing. A local chat model is a separate Discover install." },
