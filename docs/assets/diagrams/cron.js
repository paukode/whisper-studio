...
WSDiagram.mount("cron-diagram", {
  ...
  nodes: [
    { id: "create", group: "server", col: 0, row: 0, label: "Create job", sub: "POST /api/cron", desc: "A tool call or the cron panel validates the schedule and mints a job (interval, cron, or at)." },
    ...
    { id: "lease", group: "persist", kind: "store", col: 3, row: 0, label: "Lease run", sub: "cron_runs: running", desc: "start_run opens a run lease in the cron_runs table with status 'running'; boot reconciles every leftover running lease, a periodic pass uses a 15 min cutoff." },
    { id: "agent", group: "agents", col: 3, row: 1, label: "Run the turn", sub: "unified engine", desc: "An asyncio task runs the job prompt through the shared turn engine (run_turn) with TurnContext.unattended=True, so approvals execute inline and rounds are capped by cron_max_rounds." },
