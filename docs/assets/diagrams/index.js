...
WSDiagram.mount("index-diagram", {
  ...
  nodes: [
    ...
    { id: "rel", group: "index", col: 4, row: 0, label: "Relations", sub: "typed edges (optional)", desc: "Optional per-file typed relations (works_at, authored, depends_on) via Haiku or on-device Gemma. Endpoints are validated against the entity list, so the LLM cannot invent nodes." },
    ...
    { id: "rerank", group: "index", col: 2, row: 1, label: "Rerank", sub: "cross-encoder (opt)", desc: "Optional Qwen3-Reranker cross-encoder (or Cohere Rerank on Bedrock) rescores each (question, passage) pair. Best-effort behind the rag_reranker flag, which is off by default." },
