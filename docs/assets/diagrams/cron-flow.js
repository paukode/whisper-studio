...
WSDiagram.mount("cron-flow-diagram", {
  ...
  nodes: [
    ...
    { id: "agent", group: "agents", col: 4, row: 0, label: "Runs the prompt", sub: "same engine as chat", desc: "The saved prompt runs through the same unified turn engine and partitioned tool pool as chat, on the Anthropic model the job pins, or Haiku and then Sonnet as the fallback (a cron run does not inherit the chat session's model, and on-device models are refused for now), and produces a concise status report." },
