...
WSDiagram.mount("goals-tut-diagram", {
  ...
  nodes: [
    ...
    { id: "goal", group: "model", col: 1, row: 1, label: "/goal <text>", sub: "run until done", desc: "A multi-step task where you want Whisper to keep going until it is genuinely finished, not stop halfway. The command sets the goal AND sends the same text as the first message, so work starts at once; each turn is then checked against the goal." },
    ...
    { id: "gate", group: "security", col: 2, row: 1, label: "completion gate", sub: "achieved?", desc: "At the end of each turn the gate runs Stop hooks, checks the deliverables the reply claims, checks that something green ran after the last code edit, runs any commands you added with /goal gate add, then asks a cheap evaluator (which weighs a VERIFY PASS/FAIL from the built-in verify_change tool above prose). If the goal is not met, it feeds back what is missing and the turn continues." },
    { id: "done", group: "persist", col: 3, row: 1, kind: "store", label: "done", sub: "goal achieved (capped)", desc: "When the evaluator says the goal is achieved, the turn ends and the goal stops gating. A shared consecutive-block cap (default 8) backstops a stuck task so it can never loop forever." }
  ],
  edges: [
