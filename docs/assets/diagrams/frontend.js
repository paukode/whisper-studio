...
WSDiagram.mount("frontend-diagram", {
  ...
  nodes: [
    { id: "shell", group: "browser", col: 0, row: 0.5, label: "AppShell", sub: "layout + init", desc: "Top-level layout (sidebar, chat, workspace, dock) plus the startup sequence: load config, models, retention, skills, MCP, restore sessions, check workspace status." },
    ...
    { id: "stores", group: "browser", col: 2, row: 0.5, label: "Zustand stores", sub: "chat · ws · ui", desc: "Per-session factories (chat, transcription) plus singletons (session, workspace, ui, settings, layout, dock, recording, task, cronUnread, subagent). Zustand v5: never return a fresh object from a selector." },
    ...
    { id: "sse", group: "transport", col: 2, row: 1.5, label: "readSSEStream", sub: "route by type", desc: "Parses the /api/chat SSE frames and dispatches each event (text, thinking, skill_result, approval_request, team_progress, usage, grounding) into the owning session's stores." },
